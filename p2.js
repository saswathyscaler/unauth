...
exports.getProjectWeather = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const weatherResponse = await axios.get(` https://api.openweathermap.org/data/2.5/forecast?zip=${zip},US&units=imperial&appid=${appid}`);

    //  ` https://api.openweathermap.org/data/2.5/forecast?zip={zi$p},US&units=imperial&appid=${appid}`

      // Respond with the data
      res.json({ geoCodeResponse: geoCodeResponse.data, weatherResponse: weatherResponse.data });
      console.log(geoCodeResponse.data.list.main.wind,"geoCodeResponse.data")
